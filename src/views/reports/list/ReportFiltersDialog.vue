<template>
  <c-dialog v-bind="$attrs" v-model="dialogVisible" @open="initFiltersForm">
    ...
    <el-form v-show="!loading" ref="formRef" :model="form" label-position="top">
      ...
      <el-form-item label="生徒" prop="student">
        <el-input v-model="form.student" placeholder="生徒"></el-input>
      </el-form-item>
      ...
      <el-form-item label="既読・未読" prop="is_read">
        <el-select
          ...
          class="w-full"
        >
          <el-option label="既読" :value="1"></el-option>
          <el-option label="未読" :value="0"></el-option>
        </el-select>
      </el-form-item>
...
<script lang="ts" setup>
  ...
  import { getNewsTeachers, getSchoolList } from '@/services';
  ...
  /* data */
  const schoolOptions = ref<OptionType<SchoolOptions>[]>([]);
  const teacherOptions = ref<OptionType<TeacherOptions>[]>([]);
  ...
  const searchTeachers = async (keyword?: string) => {
    ...
    try {
      ...
      const { items } = await getNewsTeachers({
        ...(keyword ? { keyword } : {}),
        pagesize: MaxPageSize,
      });
      teacherOptions.value = items.map(({ id, name }) => ({
        label: `${id} | ${name}`,
        value: id,
      }));
    } finally {
  ...
  const initFiltersForm = async () => {
    try {
      loading.value = true;
      await Promise.all([searchSchools(), searchTeachers()]);
    } finally {
      loading.value = false;
    }
