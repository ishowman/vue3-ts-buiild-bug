...
<script lang="ts" setup>
  ...
  async function initialize() {
    try {
      ...
      template.value =
        ...
        (await (async () => {
          loading.value = true;
          const { items } = await getTeacherNewsTemplateList({
      ...
      // console.log(`template`, template.value, props.id)
      if (!template.value) {
        router.replace({ name: 'NewsTeacherTemplateList' });
        return;
      }
      formEntries.value = [
        {
  ...
  async function submit() {
    try {
      ...
      await teacherNewsTemplateCopy({
        template_id: Number(props.id),
        school_ids: Object.values(formData.value.schools).join(),
      });
      ElMessage.success('更新しました！');
      router.push({ name: 'NewsTeacherTemplateList' });
    } catch (_err) {
