      ...
      <template v-for="field in selectedFields" :key="field">
        ...
        >
          <template #default="scope">
            <el-button
              v-if="hasEditPermission && !scope.row.school_name"
              text
              type="primary"
...
<script lang="ts" setup>
  ...
  import TemplateFiltersDialog from './TemplateFiltersDialog.vue';
  ...
  function linkToCreate() {
    router.push({ name: 'NewsTemplateCreate' });
  }

  function linkToEdit(id) {
    router.push({ name: 'NewsTemplateEdit', params: { id } });
  }
  ...
  function multipleCopyTempalte(rowData) {
    const { id, school_name, title, date, schools } = rowData;
    router.push({
