    ...
    >
      ...
      <el-form-item prop="csvString">
        ...
        <div class="text-secondary text-[12px] leading-4">
          <p class="mb-4 mt-1">
            講師ID,権限コード,IPアドレス1,IP権限コード1,IPアドレス2,IP権限コード2…
          </p>
        </div>
      </el-form-item>
    </el-form>
...
<script lang="ts" setup>
  ...
  import {
    getTeacherRoleBatchImportConfig,
    getTeacherRoleBatchImportInfo,
    validateTeacherRoleCSVData,
    createTeacherRoleBatchImportJobs,
    getTeacherRoleBatchImportProgress,
  } from '@/services';
  ...
  const verifyResult = computed(() => {
    const total = rowData.value.length - 1; // csv 文件需要 header，所以这里 -1
  ...
  async function toNextStep(stepValue) {
    ...
    if (stepValue === 0) {
      ...
      for (let index = 0; index < chunkCount; index++) {
        ...
        batchValidations.push(await validateTeacherRoleCSVData(formData));
      }
    ...
    if (stepValue === 1) {
      ...
      const { status, progress, errors } =
        await createTeacherRoleBatchImportJobs({
  ...
  async function queryJobs() {
    ...
    const { status, progress, errors } =
      await getTeacherRoleBatchImportProgress({
        batch_id: batchId.value,
      });
