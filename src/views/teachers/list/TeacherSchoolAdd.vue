<template>
  <div class="comiru-content py-6" style="--label-width: 300px">
    ...
    >
      ...
      <el-form-item prop="csvString">
        ...
        <div class="text-secondary text-[12px] leading-4">
          <p class="mb-4 mt-1">
            講師メールアドレス,講師権限,教室1,教室2,教室3,教室4,教室5
          </p>
          <p class="mb-4 mt-1">
            例：teacher@comiru.jp,1,99999,99998,99997,99996,99995
          </p>
          <p class="mb-4 mt-1">講師権限：1=管理者,0=一般講師</p>
          <p class="mb-4 mt-1 text-red-500">
            ※Comiruに登録のない講師は追加できません
          </p>
        </div>
      </el-form-item>
    </el-form>
    ...
    <div v-if="activeStep === 2" v-loading="loading">
      <div
        v-if="
    ...
    <div v-if="tableData.length" class="mx-auto mt-7 w-fit max-w-[1200px]">
      <el-table
        v-loading="!verifyed"
...
<script lang="ts" setup>
  ...
  import {
    getTeacherSchoolBatchImportConfig,
    getTeacherSchoolBatchImportInfo,
    validateTeacherSchoolCSVData,
    createTeacherSchoolBatchImportJobs,
    getTeacherSchoolBatchImportProgress,
  } from '@/services';
  ...
  async function toNextStep(stepValue) {
    ...
    if (stepValue === 0) {
      ...
      const config = await getTeacherSchoolBatchImportConfig();
      console.log('getBatchImportConfig config', config);

      const chunkCount = Math.ceil(rowData.value.length / chunkSize);
      const { batch_id } = await getTeacherSchoolBatchImportInfo({
      ...
      for (let index = 0; index < chunkCount; index++) {
        ...
        batchValidations.push(await validateTeacherSchoolCSVData(formData));
      }
    ...
    if (stepValue === 1) {
      ...
      const { status, progress, errors } =
        await createTeacherSchoolBatchImportJobs({
  ...
  async function queryJobs() {
    ...
    const { status, progress, errors } =
      await getTeacherSchoolBatchImportProgress({
        batch_id: batchId.value,
      });
