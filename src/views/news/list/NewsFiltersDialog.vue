...
<script lang="ts" setup>
  ...
  import { NewsFieldsMap, MaxPageSize, newsStatus } from '@/constants';
  ...
  async function initFiltersForm() {
    ...
    try {
      loading.value = true;
      await Promise.all([searchSchool(), searchTeachers()]);
  ...
  async function searchTeachers(keyword?: string) {
    ...
    try {
      ...
      teacherList.value = items.map(({ id, name }) => ({
        label: name,
        value: id,
      }));
    } finally {
